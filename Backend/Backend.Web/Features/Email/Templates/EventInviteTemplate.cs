...
public class EventInviteTemplate(Event @event, string plainTextToken, string backofficeUrl) : EmailTemplateBase
{
    public override string Subject => $"Uitnodiging voor {@event.Name}";

    protected override string RenderContent()
    {
        var appLink = $"{backofficeUrl}/invite?token={plainTextToken}";
